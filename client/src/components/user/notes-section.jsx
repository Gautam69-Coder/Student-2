...
const NoteCard = memo(({ note, user, copying, onDelete, onCopy, onDownload, onPublic, onSelect, onShowCode }) => {
    ...
    return (
        <div className="bg-white dark:bg-slate-900 rounded-[10px] border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md transition-all group flex flex-col h-full overflow-hidden">
            <div className="p-5 flex flex-col h-full">
                {/* Header */}
                <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-3 min-w-0">
                        <div className={`p-2.5 rounded-[10px] flex-shrink-0 ${hasFile ? 'bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400' : 'bg-slate-900 dark:bg-slate-800 text-white'}`}>
                ...
                <div className="flex-1 mb-4">
                    {hasFile ? (
                        isImage ? (
                            <img src={note.fileData} alt={note.title} onClick={() => onSelect(note)} className="w-full h-40 object-cover rounded-[10px] border border-slate-100 dark:border-slate-800 cursor-pointer hover:opacity-90 transition-opacity" loading="lazy" />
                        ) : (
                            <div className="p-4 rounded-[10px] bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-slate-800 flex items-center gap-3">
                        ...
                        <div className="relative group/code">
                            ...
                            <div className="p-4 rounded-[10px] bg-slate-50 dark:bg-slate-950 border dark:border-slate-800 max-h-40 overflow-hidden relative">
                ...
                <div className="flex gap-2">
                    {hasFile ? (
                        ...
                        <button onClick={() => onShowCode(note)} className="w-full flex justify-center items-center gap-2 py-2 bg-slate-900 dark:bg-slate-800 text-white rounded-[10px] text-sm font-bold hover:bg-slate-800 dark:hover:bg-slate-700 shadow-sm"><Code className="w-4 h-4" /> View Code</button>
                    )}
                </div>
            </div>
        </div>
...
export function NotesSection({ notes = [], user, loading, onRefresh, requireAuth = (cb) => cb && cb() }) {
    ...
    return (
        <div className="space-y-8 pb-10">
            ...
            {/* Empty State vs List */}
            {!notes.length ? (
                <div className="flex flex-col items-center justify-center py-20 bg-white dark:bg-slate-900 rounded-[10px] border border-dashed border-slate-200 dark:border-slate-700">
            ...
            <AnimatePresence>
                {selectedNote && selectedNote.fileType !== "NAN" && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                        ...
                        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="relative w-full max-w-5xl h-[90vh] bg-white dark:bg-slate-900 rounded-[10px] shadow-2xl flex flex-col overflow-hidden border border-slate-100 dark:border-slate-800">
                            <div className="flex justify-between items-center p-4 sm:p-6 border-b border-slate-100 dark:border-slate-800">
                                ...
                                <div className="flex gap-2">
                                    ...
                                    <button onClick={() => setSelectedNote(null)} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-[10px] transition-colors"><X className="w-5 h-5 text-slate-400" /></button>
                                </div>
                            </div>
